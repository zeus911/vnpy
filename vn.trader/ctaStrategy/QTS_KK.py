...
class KkStrategy(CtaTemplate):
    ...
    bufferSize = kkLength+1                    # 需要缓存的数据的大小
    ...
    def onBar(self, bar):
        ...
        # if bar.datetime.time() > datetime.strptime("14:37:00", "%H:%M:%S").time():
        #     #平仓
        #     if self.pos > 0:
    ...
    def onFiveBar(self, bar):
        ...
        # 持有多头仓位
            ...
            orderID = self.sell(bar.close-1, abs(self.pos))
            self.orderList.append(orderID)
    
        # 持有空头仓位
        elif self.pos < 0 and bar.close > self.kkDown:
            orderID = self.cover(bar.close+1, abs(self.pos))
            self.orderList.append(orderID)                
    ...
    def onTrade(self, trade):
        ...
        if abs(self.pos) > 0:
            ...
            # 100 unit punishment for short time holding
            if self.holdTime < 40:
                self.holdTime += 100
...
if __name__ == '__main__':
    ...
    # 设置回测用的数据起始日期
    engine.setStartDate('20140401',1)
    ...
    if True:
        ...
        setting.addParameter('kkLength',15,30,1)
